...
CATEGORIES= ['Food','Transport','Utilies','Educational','Health','Leisure','Recreational']
...
def index():
    ...
    if start_date and end_date and end_date<start_date:
        flash("End date can't be befor the start date.","error")
    ...
    if end_date:
        cat_q=cat_q.filter(Expense.amount<=end_date)
    if selected_category:
    ...
    if end_date:
        day_q=day_q.filter(Expense.amount<=end_date)
    if selected_category:
        day_q=cat_q.filter(Expense.category == selected_category)
